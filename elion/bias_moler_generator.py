...
def NOT_UESD_fine_tune_moler(smiles):
    """Fine tunes the MoLeR generator with the provided SMILES

    Args:
        smiles (SMILES list): List of SMILES to finetune the generator
    """

    # this_ft = 0
    # ft_dir = Path(f"finetuning_{this_ft}")
    # while (ft_dir.exists()): 
    #     this_ft += 1
    #     ft_dir = Path(f"finetuning_{this_ft}")
    # ft_dir.mkdir(parents=True)

    # train_file = Path(ft_dir, "train.smiles")
    # valid_file = Path(ft_dir, "valid.smiles")
    # test_file  = Path(ft_dir, "test.smiles" )

    # len_train = int(0.8 * len(smiles))
    # len_valid = int((len(smiles) - len_train) // 2)
    # len_test  = len(smiles) - len_train - len_valid

    # rng = np.random.default_rng()
    # rng.shuffle(smiles, axis = 0)
    # train_smiles = smiles[:len_train]
    # valid_smiles = smiles[len_train:(len_train + len_valid)]
    # test_smiles  = smiles[(len_train + len_valid):]
    ...
    # ## CALL FINETUNING BASH SCRIPT 
    # ## with ft_dir as argument

def NOT_USED_estimate_rewards(properties:Dict,reward_params:Dict) -> Dict:
...
def NOT_USED_optimize_continuous_reward(cfg:Dict):
    pass
    # """Optimize the generator by searching for the best latent space center,
    # as determined by a continuous response function dependent on the values
    # and thresholds of the properties.

    # Args:
    #     cfg (Dict): Dictionary of configuration options
    # """
    ...
    # moler_sh  = Path(cfg['elion_root_dir'],'generators/moler/generate_moler.sh')
    ...
    #     print("Latent Space:")
    #     print(f"  ---> {'Centered at':<25s}: {center_smi}")
    #     for prop in center_predictions.keys():
    #         print(f"  ---> {prop:<25s}: {center_predictions[prop]:0.2f}")
    #     print(f"  ---> {'Reward':<25s}: {center_rew}\n")
    #     print(f"Generating a new batch of {n_batch} molecules... ", end='')
    #     result = subprocess.run([moler_sh,
    #                             "-m", moler_state, 
    #                             "-n", str(n_batch), 
    #                             "-s", template_smi, 
    #                             "-c", center_smi], capture_output=True)
    #     print("Done.")
    ...
    #     # Trying to accept only molecules with REWARD >= lowest_rew.
    #     n_improved_mols = np.sum(rew_np >= lowest_rew)
    #     print("Current batch:")
    ...
    #         # Use only the current predictions
    ...
    #         #      keeping the old SMILES. This will make the kept smiles random within
    ...
    #     print(f"  --> Randomly choosing one as the new latent space center\n")
    ...
    #                     + sep.join(f"{history[key][optimization_iteration-gen_start-1]:0.3f}" for key in history.keys()) 
    #                     + '\n')
    #         f.write(out_str)
...
def bias_generator(cfg:Dict):
    ...
    while( (not all_converged) and optimization_iteration < final_iteration):
        ...
        print("-"*80, flush=True)
        while len(generated_smi) < n_batch:
            this_center = 0
            for center_idx in range(len(max_mols)):
                center_smi = max_mols[center_idx]
                center_rew = max_rews[center_idx]
                center_idx = max_idxs[center_idx]

                for prop in best_predictions.keys():
                    center_predictions[prop] = best_predictions[prop][center_idx]

                print(f"\nGenerating a new minibatch of {n_batch//len(max_mols)} molecules: ")
                print(f"Latent Space {this_center} of {len(max_mols)}:")
                print(f"  ---> {'Centered at':<25s}: {center_smi}")
                ...
                result = subprocess.run([moler_sh,
                                        "-m", moler_state, 
                                        "-n", str(n_batch//len(max_mols)), 
                                        "-s", template_smi, 
                                        "-c", center_smi], capture_output=True)

                smiles_cur = result.stdout.decode('utf-8').split()
                n_generated = n_generated + len(smiles_cur)
                generated_smi = generated_smi.union(smiles_cur)
                print(f"Accumulated {len(generated_smi)} unique molecules so far, out of {n_batch} requested.")
                this_center = this_center + 1
                if len(generated_smi) >= n_batch: break

        #smiles_cur = np.unique(np.array(generated_smi))
        smiles_cur = np.array(list(generated_smi))
        print(f"Done. Obtained {len(smiles_cur)} unique molecules ",
              f"from a total of {n_generated} molecules generated.")
        n_best = min(max_size,len(smiles_cur))
