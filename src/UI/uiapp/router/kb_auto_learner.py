...
When route_ui_action() returns None (no KB match), this module:
  ...
  4. Appends it to:
       - uiapp/.qwen/attn_action_kb.md  OR  vina_action_kb.md  (shadow)
...
─────────────────
  ...
  To write to a dedicated file, add to app.py before app.run():
      import logging
      fh = logging.FileHandler('uiapp/.qwen/autolearn.log')
...
# ── Paths ─────────────────────────────────────────────────────────────────────
# Centralised in uiapp.config so the shadow KB and the router's main KB always
# agree on a location, independent of the current working directory.
from uiapp import config as _cfg

# Shadow KB: auto-generated, human-auditable
_QWEN_DIR    = _cfg.KB_SHADOW_DIR
os.makedirs(_QWEN_DIR, exist_ok=True)
ATTN_KB_PATH = _cfg.ATTN_ACTION_KB_PATH
VINA_KB_PATH = _cfg.VINA_ACTION_KB_PATH
...
COT_MAIN_LOG_PATH    = _cfg.COT_MAIN_LOG_PATH
AUTOLEARN_LOG_PATH   = _cfg.AUTOLEARN_LOG_PATH
...
def _call_qwen_cot(prompt: str) -> dict | None:
    ...
    """
    try:
        from uiapp.llm.qwen_client import qwen_compat, QwenParams
...
def _hot_inject(btn_id: str, action: str, response: str,
    ...
    """
    try:
        from uiapp.router.ui_action_router import ElionUIRouter
