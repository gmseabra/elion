...
class ThompsonSampler:
    ...
    # -------------------------------------------------------------------------
    # Standard single-step search() scores ONE molecule per cycle, so the
    # ChemBERT estimator always runs on a batch of 1 — a tiny matmul that cannot
    # use more than ~1 CPU core regardless of torch thread settings. As the
    # posteriors sharpen this is the entire reason observed CPU utilisation
    # decays over a long run (e.g. ~660% early, falling toward ~85% late): it is
    # not a stall, it is the per-iteration workload shrinking to a single
    # molecule.
    #
    # search_batched_ts() instead draws B independent Thompson samples against
    # the CURRENT (frozen) posterior each step, assembles all B molecules, and
    # scores them in ONE evaluate_batch() call — the same batched path warmup
    # already uses successfully. One combined posterior update is applied per
    # step. This keeps all cores fed for the whole run and finishes in far fewer
    # steps. Each of the B picks is an independent draw from the belief, so this
    # is statistically standard batched Thompson Sampling.
    #
    # Selection draws do NOT commit to the DisallowTracker mid-batch (that would
    # permanently retire B combos per step); identical combos within a batch are
    # de-duplicated in memory, and only the unique survivors are committed once.
    # =========================================================================
    def _select_one_ts_nocommit(self, rng):
        """One Thompson-sampling selection across all components, sampled against
        the current (frozen) posterior. Honours the tracker's existing disallow
        mask but does NOT call tracker.update() — the caller commits the unique
        survivors of the whole batch once.

        :return: list of reagent indices (one per component), or None if some
                 component slot is fully disallowed (space locally exhausted).
        """
        selected_reagents = [DisallowTracker.Empty] * len(self.reagent_lists)
        for cycle_id in random.sample(range(0, len(self.reagent_lists)), len(self.reagent_lists)):
            reagent_list = self.reagent_lists[cycle_id]
            ...
            if np.all(np.isnan(choice_row)):
                return None  # this slot is fully disallowed → combo unavailable

            winner_idx = self.pick_function(choice_row)
            selected_reagents[cycle_id] = int(winner_idx)
        return selected_reagents

    def search_batched_ts(self, num_cycles=25, ts_batch_size=128, log_batch_size: int = 1):
        """Batched Thompson Sampling search (see block comment above).

        :param num_cycles: number of batched steps to run. NOTE: each step now
                           scores up to ts_batch_size molecules, so this scores
                           far more molecules than the same num_cycles of the
                           single-step search(). Size accordingly.
        :param ts_batch_size: B — number of independent Thompson draws (molecules)
                              to assemble and score per step. Should match the
                              estimator batch_size (ChemBERT uses 128) so the
                              batch becomes one full multi-core matmul.
        :param log_batch_size: molecules to collect before printing as a debug
                               batch (log readability only).
        :return: list of [score, smiles, name] results.
        """
        out_list = []
        rng = np.random.default_rng()
        log_batch: list[tuple[int, float, str, str]] = []

        self.logger.info(
            '[batched-TS] starting: num_cycles=%d, ts_batch_size=%d '
            '(scores up to %d molecules per step in one evaluate_batch call)',
            num_cycles, ts_batch_size, ts_batch_size)

        for i in tqdm(range(0, num_cycles), desc="BatchCycle", disable=self.hide_progress):
            # ── draw B independent posterior samples against the frozen belief ──
            seen: set = set()
            batch_choices: list[list[int]] = []
            none_draws = 0
            for _ in range(ts_batch_size):
                sel = self._select_one_ts_nocommit(rng)
                if sel is None:
                    none_draws += 1
                    continue
                key = tuple(sel)
                if key in seen:
                    continue  # intra-batch dedupe (in-memory, not the tracker)
                seen.add(key)
                batch_choices.append(sel)

            if not batch_choices:
                self.logger.info(
                    '[batched-TS] step %d produced no available combinations '
                    '(combination space exhausted) — stopping at %d molecules.',
                    i, len(out_list))
                break

            # ── commit the unique survivors to the tracker ONCE ──
            for sel in batch_choices:
                self._disallow_tracker.update(sel)

            # ── score all unique candidates in ONE evaluate_batch() call ──
            # evaluate_batch returns (smiles, name, score, selected_reagents) per
            # entry, in order, with failed reactions scored nan.
            batch_results = self.evaluate_batch(batch_choices)

            self.logger.debug(
                '[batched-TS] step %d: %d unique candidates scored in one batch '
                '(%d null draws, %d intra-batch dupes)',
                i, len(batch_choices), none_draws,
                ts_batch_size - none_draws - len(batch_choices))

            # ── collect valid results and stage one combined posterior update ──
            pending_batch: list[tuple[list, float]] = []
            for smiles, name, score, sel_reagent_objs in batch_results:
                if np.isfinite(score):
                    out_list.append([score, smiles, name])
                    pending_batch.append((sel_reagent_objs, score))
                    log_batch.append((i, score, smiles, name))

            # ── single posterior update for the whole batch ──
            if pending_batch:
                self._flush_score_batch(pending_batch)
                if self.logger.isEnabledFor(logging.DEBUG):
                    # Log post-update state for the reagents actually touched
                    # (deduped by identity), not a repeated single selection.
                    seen_ids: set = set()
                    for sel_r_objs, _ in pending_batch:
                        for r in sel_r_objs:
                            if id(r) in seen_ids:
                            ...
                            seen_ids.add(id(r))
                            self.logger.debug(
                                'post-update %s μ=%.6f σ=%.6f n=%d',
                                r.reagent_name, r.current_mean,
                                r.current_std, r.num_scores)

            # ── batched debug log of molecules (readability) ──
            if len(log_batch) >= log_batch_size or i == num_cycles - 1:
                if log_batch:
                    self.logger.debug(
                        '=== batched-TS [step %d] | %d molecules ===',
                        i, len(log_batch))
                    for b_iter, b_score, b_smiles, b_name in log_batch:
                        self.logger.debug(
                            '  iter=%d | score=%.6f | smiles=%s | name=%s',
            ...
            if i % 100 == 0 and out_list:
                top_score, top_smiles, top_name = self._top_func(out_list)
                self.logger.info(
                    'BatchStep: %d | molecules so far: %d | max score: %.6f | '
                    'smiles: %s | name: %s',
                    i, len(out_list), top_score, top_smiles, top_name)

        self.logger.info(
            '[batched-TS] finished: %d steps, %d molecules scored.',
            i + 1, len(out_list))
        return out_list