...
# ZccE
from rdkit import Chem
ligand = Chem.MolFromPDBFile('/blue/lic/huangzihang/repos/PretrainDrugDiscovery-main/data/toy_set/ZccE/ZccE_ligand.pdb', removeHs=True)
pocket = Chem.MolFromPDBFile('/blue/lic/huangzihang/repos/PretrainDrugDiscovery-main/data/toy_set/ZccE/ZccE_protein.pdb', removeHs=True)
complex = (ligand, pocket)
with open('/blue/lic/huangzihang/repos/PretrainDrugDiscovery-main/data/toy_set/ZccE/ZccE_5A.rdkit', 'wb') as f:
    pickle.dump(complex, f)
...
toy_df = pd.read_csv(os.path.join(toy_dir, "ZccE.csv"))
...
toy_set_loader = PLIDataLoader(toy_set, batch_size=1, shuffle=True, num_workers=4)
# toy_set_rmse, toy_set_coff = val(model, toy_set_loader, device)
val(model, toy_set_loader, device)
