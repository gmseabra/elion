...
def inter_graph(ligand, pocket, dis_threshold = 5.):
    ...
    edges = list(graph_inter.edges(data=False))
    if len(edges) > 0:
        edge_index_inter = torch.stack([torch.LongTensor((u, v)) for u, v in edges]).T
    else:
        # Create an empty tensor with the correct shape (2, 0)
