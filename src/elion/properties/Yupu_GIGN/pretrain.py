...
if __name__ == '__main__':
    ...
    for repeat in range(repeats):
        ...
        train_set = PreGraphDatasetD("/blue/zhe.jiang/y.zhang1/proteinData/pretrain521", train_df, graph_type=graph_type, create=False)
        valid_set = PreGraphDatasetD("/blue/zhe.jiang/y.zhang1/proteinData/pretrain521", valid_df, graph_type=graph_type, create=False)
        ...
        for epoch in range(epochs):
            ...
            for data in train_loader:
                ...
                loss =0.5*loss+loss1
                pred2,label2,x12 = model(InputBatch2)
                loss2 = 1.2*denoising_loss(pred2,InputBatch2,label2,2,batch_size,criterion2)
                loss += loss2
                # break
                optimizer.zero_grad()
                loss.backward()
        ...
        device = 'cuda0'
        valid_rmse, valid_pr = val(model,model1, valid_loader,'cuda0')
        # test2013_rmse, test2013_pr = val(model,model1, test2013_loader,'cuda0')
        # test2016_rmse, test2016_pr = val(model,model1, test2016_loader,'cuda0')
        # test2019_rmse, test2019_pr = val(model,model1, test2019_loader,'cuda0')
