...
def generate_complex(data_dir, data_df, distance=5, input_ligand_format='mol2'):
    ...
    for i, row in data_df.iterrows():
        ...
        print('------------debug1 ligand_path: %s' % ligand_path)
        ligand = Chem.MolFromPDBFile(ligand_path, removeHs=False)
        if ligand == None:
        ...
        # test = torch.load(f"/blue/lic/huangzihang/repos/PretrainDrugDiscovery-main/data/train/{cid}/Graph_GIGN-{cid}_5A.pyg")
        # print(test)
        if pocket == None:
...
if __name__ == '__main__':
    ...
    data_root = '/blue/lic/huangzihang/repos/PretrainDrugDiscovery-main/data/toy_set'
    data_dir = data_root
    data_df = pd.read_csv(os.path.join('./data', 'test_process.csv'))
