...
''', formatter_class=argparse.RawTextHelpFormatter)
    ...
    if not input_has_hydrogens:
        ...
        def find_bio_atom(ob_atom, bio_atoms, serial_to_bio, tolerance=0.01):
            ...
            # Coordinate-based matching with element instead of name
            ob_coord = array([ob_atom.GetX(), ob_atom.GetY(), ob_atom.GetZ()])
            ob_atomic_num = ob_atom.GetAtomicNum()
            ob_element = ATOMIC_NUMBER_TO_ELEMENT.get(ob_atomic_num, '')  # Use reverse dict
            
            for bio_atom in bio_atoms:
                bio_coord = bio_atom.get_coord()
                distance = sqrt(sum((ob_coord - bio_coord) ** 2))
                bio_element = bio_atom.element.strip().upper()
                if distance < tolerance and bio_element == ob_element:
                    return bio_atom
            return None
    ...
    # print('-----------------debug2 pdb_filename: %s' % pdb_filename)
    # print('-----------------debug2 atomtypes_filename: %s' % atomtypes_filename)
    # print('-----------------debug2 outdir: %s' % outdir)
