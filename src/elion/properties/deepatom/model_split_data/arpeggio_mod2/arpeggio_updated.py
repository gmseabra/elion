...
# import inspect
# print inspect.getsourcefile(PDBParser)  # Get the source file path of PDBParser
# import sys
# sys.exit(1)
# print Bio.PDB.__file__  # Print the Bio.PDB module path
...
from config import ATOM_TYPES, CONTACT_TYPES, VDW_RADII, METALS, \
                   ...
                   ANOLEA_ATOM_TYPES, Arpeggio_ATOM_TYPES, LIGAND_ELEMENT_TYPES
...
''', formatter_class=argparse.RawTextHelpFormatter)
    ...
    print('Mapped OB to BioPython atoms and vice-versa.')
    ...
    for residue in s.get_residues():
        ...
        if residue.resname in STD_RES:
            ...
            for atom in residue.child_list:
                # print 'atom.atom_types: %s' % str(atom.atom_types)
                # print(atom.get_id())
    ...
    def make_anolea_string(atom):
        anolea_atomtype = "@@"  # to check for non-supported ANOLEA atom types
        ...
        try:
            ...
            if chain_id=='y':
                anolea_atomtype = LIGAND_ELEMENT_TYPES[get_ligand_element(atom)]
                # print 'atom.atom_types: %s' % str(atom.atom_types)
                # print(atom.get_id())
                # print(atom.get_coord())
                # print(atom.get_serial_number())
    ...
    def get_ligand_element(atom):
        # PDB atom names are upper-case (e.g. 'CL1', 'BR2'), but the keys in
        # LIGAND_ELEMENT_TYPES use standard chemical capitalisation
        # ('Cl', 'Br', ...). Normalise so two-letter elements line up with the
        # table -- 'CL' -> 'Cl', 'BR' -> 'Br' -- while single-letter elements
        # ('C', 'N', 'O', 'F', 'P', 'S', 'I') stay upper-case.
        name = atom.name.strip()
        if len(name) >= 2 and name[1].isalpha():
            lig_element = name[:2].capitalize()
        else:
            lig_element = name[:1].upper()
    ...
    def make_arpeggio_string(atom):
        arpeggio_string = ""
        # print 'Arpeggio_ATOM_TYPES: %s' % str(Arpeggio_ATOM_TYPES)
    ...
    with open(atomtypes_file, 'w') as fo:
        for atom in s_atoms:
            ...
            if residue.get_id()[0] != 'W' and make_anolea_string(atom)!="@@":
                ...
                # print '{}{}{}{}{}{}\n'.format(pymol_string, anolea_string, arpeggio_string, coord_x_str, coord_y_str, coord_z_str)
