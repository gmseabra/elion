...
# Map ligand atom types to anolea_vdw_dict keys
ANOLEA_ATOM_TYPES = {
    'LIGC1': '41',   # Carbon
    'LIGN1': '42',   # Nitrogen
    'LIGO1': '43',   # Oxygen
...
# Map elements to anolea_vdw_dict keys
LIGAND_ELEMENT_TYPES = {
    'C': '41',   # Carbon
    'N': '42',   # Nitrogen
    'O': '43',   # Oxygen
    'F': '44',   # Fluorine
...
def make_pymol_string(entity):
    ...
    chain_id_str = "%-3s" % chain.id
    res_num_str = "%-8s" % res_num
    res_name_str = "%-6s" % res_name
    atom_name_str = "%-5s" % atom_name

    
    macro =  '{}{}{}{}'.format(chain_id_str,
                               res_num_str,
                               res_name_str,
                               atom_name_str)
...
def is_hbond(donor, acceptor):
    ...
    for hydrogen_coord in donor.h_coords:
        ...
        # Check distance condition: sum of VDW radii (no tolerance needed for basic check)
        if h_dist <= VDW_RADII['H'] + acceptor.vdw_radius:
...
''', formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument('outdir', type=str, help='Path to the *.atomtypes file to be written.')
    ...
    print('pdb_filename: %s' % pdb_filename)
    ...
    print('Loaded PDB structure (OpenBabel)')

    # Count total OpenBabel atoms for later reporting
    total_ob_atoms = mol.NumAtoms()
    logging.debug(f"OpenBabel loaded {total_ob_atoms} atoms from the PDB file.")
    print(f"Total OpenBabel atoms: {total_ob_atoms}")
    ...
    for ob_atom in ob.OBMolAtomIter(mol):
        ...
        try:
            ...
            logging.warning(f"OpenBabel atom serial {serial} not found in BioPython structure. Skipping...")
            print(f"WARNING: Skipping OpenBabel atom {serial} (no BioPython match)")
            continue

    # Log successful mapping
    mapped_count = len(ob_to_bio)
    skipped_count = total_ob_atoms - mapped_count
    logging.info(f'Mapped {mapped_count} OB to BioPython atoms (skipped {skipped_count} due to mismatches).')
    print(f'Mapped {mapped_count}/{total_ob_atoms} OB to BioPython atoms successfully (skipped {skipped_count}).')

    if skipped_count > 0:
        print(f"Note: {skipped_count} atoms were skipped due to parsing mismatches between OpenBabel and BioPython.")
        logging.info(f"Parsing mismatches may be due to TER records, duplicate atoms, or HETATM handling differences.")

    # INSERT DEBUGGING CODE HERE
    for ob_atom in ob.OBMolAtomIter(mol):
        bio_atom = ob_to_bio.get(ob_atom.GetId())
        if bio_atom:
            if bio_to_ob.get(bio_atom) != ob_atom.GetId():
                logging.warning(f"Mapping inconsistency: bio_to_ob[{bio_atom}] = {bio_to_ob.get(bio_atom)}, expected {ob_atom.GetId()}")
        else:
            logging.warning(f"No BioPython atom mapped for OpenBabel atom {ob_atom.GetId()} (element {ob_atom.GetType()})")

    for bio_atom in s.get_atoms():
        ob_id = bio_to_ob.get(bio_atom)
        if ob_id is not None:
            if ob_to_bio.get(ob_id) != bio_atom:
                logging.warning(f"Mapping inconsistency: bio_to_ob[{bio_atom}] = {ob_id}, but ob_to_bio[{ob_id}] = {ob_to_bio.get(ob_id)}")
        else:
            logging.warning(f"No OpenBabel atom mapped for BioPython atom {bio_atom.name} (element {bio_atom.element})")

    logging.info(f"Completed atom mapping. bio_to_ob size: {len(bio_to_ob)}, ob_to_bio size: {len(ob_to_bio)}")

    # ADD EMPTY DATA STRUCTURES FOR TAGGED ATOM DATA
    for atom in s_atoms:
        atom.atom_types = set([])
        atom.h_coords = []
        atom.is_metal = atom.element.upper() in METALS
        atom.is_halogen = atom.element.upper() in HALOGENS
    ...
    if not input_has_hydrogens:
        ...
        # REBUILD MAPPINGS AFTER HYDROGEN ADDITION USING COORDINATES AND SERIAL FALLBACK
        logging.info("Rebuilding OB to BioPython mappings after hydrogen addition...")
        ob_to_bio = {}
        bio_to_ob = {}
        
        def find_bio_atom(ob_atom, bio_atoms, serial_to_bio, tolerance=0.5):
            from numpy import array, sqrt
            try:
                # Try serial number first for non-hydrogen atoms
                serial = ob_atom.GetResidue().GetSerialNum(ob_atom)
                if serial in serial_to_bio and ob_atom.GetAtomicNum() != 1:  # Exclude hydrogens
                    return serial_to_bio[serial]
            except:
                pass
            
            # Coordinate-based matching with relaxed criteria
            ob_coord = array([ob_atom.GetX(), ob_atom.GetY(), ob_atom.GetZ()])
            ob_name = ob_atom.GetType().split()[0]
            try:
                ob_resnum = ob_atom.GetResidue().GetNum()
                ob_resname = ob_atom.GetResidue().GetName()
            except:
                ob_resnum = None
                ob_resname = None
            
            for bio_atom in bio_atoms:
                bio_coord = bio_atom.get_coord()
                distance = sqrt(sum((ob_coord - bio_coord) ** 2))
                bio_name = bio_atom.name.strip()
                bio_resnum = bio_atom.get_parent().id[1]
                bio_resname = bio_atom.get_parent().resname
                if (distance < tolerance and
                    bio_name == ob_name and
                    (ob_resnum is None or bio_resnum == ob_resnum or ob_atom.GetAtomicNum() == 1) and
                    (ob_resname is None or bio_resname == ob_resname or ob_atom.GetAtomicNum() == 1)):
                    return bio_atom
            return None

        s_atoms = list(s.get_atoms())  # Refresh BioPython atoms
        serial_to_bio = {x.serial_number: x for x in s_atoms}
        total_ob_atoms_after = mol.NumAtoms()
        mapped_after = 0
        skipped_after = 0
        skipped_ids = []
        
        for ob_atom in ob.OBMolAtomIter(mol):
            bio_atom = find_bio_atom(ob_atom, s_atoms, serial_to_bio)
            if bio_atom:
                ob_to_bio[ob_atom.GetId()] = bio_atom
                bio_to_ob[bio_atom] = ob_atom.GetId()
                mapped_after += 1
            else:
                ob_id = ob_atom.GetId()
                skipped_ids.append(ob_id)
                logging.debug(f"No BioPython mapping for OpenBabel atom {ob_id} (element {ob_atom.GetAtomicNum()})")
                skipped_after += 1
        
        logging.info(f"Post-hydrogen mapping: {mapped_after}/{total_ob_atoms_after} atoms mapped (skipped {skipped_after})")
        print(f"Rebuilt mappings: {mapped_after}/{total_ob_atoms_after} atoms (skipped {skipped_after})")
        if skipped_after > 0:
            print(f"Skipped atom IDs after hydrogen addition: {skipped_ids[:5]}...")
    ...
    for atom_type, smartsdict in ATOM_TYPES.items():
        for smarts in smartsdict.values():
            ...
            for match in matches:
                ...
                ob_id = atom.GetId()
                if ob_id in ob_to_bio:
                    try:
                        ob_to_bio[ob_id].atom_types.add(atom_type)
                    except AttributeError:
                        logging.warning(f"BioPython atom {ob_id} missing atom_types attribute. Initializing...")
                        ob_to_bio[ob_id].atom_types = set([atom_type])
                else:
                    logging.warning(f"No BioPython mapping for OpenBabel atom {ob_id} (element {atom.GetAtomicNum()}). Skipping atom type assignment.")
    ...
    # DEFENSIVE ACCESS FUNCTION
    def get_mapped_bio_atom(ob_atom, ob_to_bio, logger):
        ob_id = ob_atom.GetId()
        bio_atom = ob_to_bio.get(ob_id)
        if not bio_atom:
            logger.warning(f"Skipping OpenBabel atom {ob_id} (element {ob_atom.GetAtomicNum()}) - no BioPython mapping")
        return bio_atom

    # ELEMENT TO ATOMIC NUMBER MAPPING  <-- PASTE THE BLOCK HERE
    ELEMENT_TO_ATOMIC_NUMBER = {
        'H': 1, 'HE': 2, 'LI': 3, 'BE': 4, 'B': 5, 'C': 6, 'N': 7, 'O': 8, 'F': 9, 'NE': 10,
        'NA': 11, 'MG': 12, 'AL': 13, 'SI': 14, 'P': 15, 'S': 16, 'CL': 17, 'AR': 18,
        'K': 19, 'CA': 20, 'SC': 21, 'TI': 22, 'V': 23, 'CR': 24, 'MN': 25, 'FE': 26,
        'CO': 27, 'NI': 28, 'CU': 29, 'ZN': 30, 'GA': 31, 'GE': 32, 'AS': 33, 'SE': 34,
        'BR': 35, 'KR': 36  # Add more as needed
    }
    # ADD THIS DICTIONARY:
    # VALENCE ELECTRON DICTIONARY (overwrites incorrect import from 'config')
    VALENCE = {
        1: 1,  # H
        5: 3,  # B
        6: 4,  # C
    ...
    # LONE PAIR CALCULATION (around line 1179)
    for bio_atom in s_atoms:
        lone_pairs = 0  # Default value
        ob_id = bio_to_ob.get(bio_atom)
        if ob_id is None:
            logging.warning(f"No OpenBabel atom for BioPython atom {bio_atom.name} (element {bio_atom.element}). Skipping lone pair calculation.")
        else:
            ob_atom = mol.GetAtomById(ob_id)
            element = bio_atom.element.upper()
            if element not in ELEMENT_TO_ATOMIC_NUMBER:
                logging.warning(f"Unknown element {element} for BioPython atom {bio_atom.name}. Skipping lone pair calculation.")
            else:
                atomic_number = ELEMENT_TO_ATOMIC_NUMBER[element]
                bond_order = sum(bond.GetBondOrder() for bond in ob.OBAtomBondIter(ob_atom))
                formal_charge = ob_atom.GetFormalCharge()
                lone_pairs = VALENCE.get(atomic_number, 0) - bond_order - formal_charge
                logging.debug(f"Atom {bio_atom.name}: atomic_number={atomic_number}, bond_order={bond_order}, formal_charge={formal_charge}, lone_pairs={lone_pairs}")
        
        # At line 1191, use lone_pairs safely
        if lone_pairs != 0:
            # Continue with lone pair processing (e.g., store or use for interactions)
            bio_atom.lone_pairs = lone_pairs  # Example: store lone_pairs
        else:
            bio_atom.lone_pairs = 0  # Ensure attribute is set

    # DEFENSIVE ACCESS FUNCTION FOR LATER USE
    def get_mapped_bio_atom(ob_atom, ob_to_bio, logger):
        ob_id = ob_atom.GetId()
        bio_atom = ob_to_bio.get(ob_id)
        if not bio_atom:
    ...
    def make_anolea_string(atom):
        anolea_atomtype = "@@"  # Default for unsupported atom types
        residue = atom.get_parent()
        chain_id = residue.get_parent().id
        element = get_ligand_element(atom)  # Use the updated function
        
        # Log the atom being processed
        logging.debug("Processing atom {0} in residue {1} (chain {2}) with element {3} at coordinates {4}".format(
            atom.name, residue.resname, chain_id, element, atom.coord))
        
        # Try to map to ANOLEA atom types for protein residues
        anolea_key = residue.get_resname().upper() + atom.get_name().upper()
        try:
            anolea_atomtype = ANOLEA_ATOM_TYPES[anolea_key]
            logging.debug("Mapped to ANOLEA atom type: {0} for protein residue {1} atom {2}".format(
                anolea_atomtype, residue.resname, atom.name))
        except KeyError:
            # Check if it's a ligand (resname 'UNL' or chain 'y')
            if residue.resname.upper() == 'UNL' or chain_id == 'y':
                try:
                    anolea_atomtype = LIGAND_ELEMENT_TYPES[element]
                    logging.debug("Mapped to ligand element type: {0} for element {1}".format(
                        anolea_atomtype, element))
                except KeyError:
                    # Handle missing elements like 'CL' and 'F'
                    if element == 'CL':
                        anolea_atomtype = 'CL'  # Explicitly set for chlorine
                        logging.info("Assigned 'CL' to chlorine atom {0} in ligand (chain {1})".format(
                            atom.name, chain_id))
                    elif element == 'F':
                        anolea_atomtype = 'F'  # Explicitly set for fluorine
                        logging.info("Assigned 'F' to fluorine atom {0} in ligand (chain {1})".format(
                            atom.name, chain_id))
                    else:
                        logging.warning("Element {0} not found in LIGAND_ELEMENT_TYPES for atom {1} in residue {2} (chain {3}). Skipping with '@@'".format(
                            element, atom.name, residue.resname, chain_id))
                        anolea_atomtype = "@@"  # Skip unsupported elements
        
        # Log if the atom is skipped
        if anolea_atomtype == "@@":
            logging.info("Skipping atom {0} (element {1}) in residue {2} (chain {3}) with '@@'".format(
                atom.name, element, residue.resname, chain_id))
        
        return anolea_atomtype


    def get_ligand_element(atom):
        element = atom.element.strip().upper()  # Use BioPython's element field directly
        if not element:  # Fallback if element field is empty
            atom_name = atom.name.strip()
            if len(atom_name) >= 2 and atom_name[:2].isalpha():
                element = atom_name[:2].upper()  # e.g., "CL" for chlorine
            else:
                element = atom_name[0].upper()  # e.g., "C" for carbon
            logging.warning("Element not explicitly defined for atom {0} in residue {1}. Inferred element: {2}".format(
                atom.name, atom.get_parent().resname, element))
        
        # Log the element being processed
        logging.debug("Processing element: {0} for atom {1} at coordinates {2}".format(
            element, atom.name, atom.coord))
        
        return element

    def make_arpeggio_string(atom):
        arpeggio_string = ""
        for a_type in Arpeggio_ATOM_TYPES:
            if a_type in atom.atom_types:
                arpeggio_string += "1"
            else:
                arpeggio_string += "0"
    ...
    atomtypes_filename = pdb_filename.replace('.pdb',  '.atomtypes')
    atomtypes_file = os.path.join(outdir, atomtypes_filename)

    # print('-----------------debug2 atomtypes_file: %s' % atomtypes_file)
    with open(atomtypes_file, 'w') as fo:
        for atom in s_atoms:
            residue = atom.get_parent()
            x, y, z = atom.get_coord()
            # ignore WATER and non-supported ANOLEA atom types
            if residue.get_id()[0] != 'W' and make_anolea_string(atom)!="@@":

                pymol_string = make_pymol_string(atom)
                anolea_string = "%2s" % make_anolea_string(atom)
                arpeggio_string = "%15s" % make_arpeggio_string(atom)
                coord_x_str = str("%10.3f" % x)
                coord_y_str = str("%10.3f" % y)
                coord_z_str = str("%10.3f" % z)

                fo.write('{}{}{}{}{}{}\n'.format(pymol_string, anolea_string,
                    arpeggio_string, coord_x_str, coord_y_str, coord_z_str))
    ...
    for ob_atom in ob.OBMolAtomIter(mol):
        ...
        valence = sum(bond.GetBondOrder() for bond in ob.OBAtomBondIter(ob_atom))
        ...
        bio_atom = get_mapped_bio_atom(ob_atom, ob_to_bio, logging)
        if bio_atom:
            # Continue processing (e.g., bio_atom.h_coords.append(...))
            pass
        else:
            continue
    ...
    for atom in s_atoms:
        ...
        if 'hbond acceptor' in atom.atom_types or 'hbond donor' in atom.atom_types:
            ...
            if 'hbond acceptor' in atom.atom_types:
                ...
                for bio_atom in s_atoms:  # or whatever loop contains line 1179
                    ob_id = bio_to_ob.get(bio_atom)
                    if ob_id is None:
                        logging.warning(f"No OpenBabel atom for BioPython atom {bio_atom.name} (element {bio_atom.element}). Skipping lone pair calculation.")
                        continue
    ...
    for atom in entity:
        atomic_num = mol.GetAtomById(bio_to_ob[atom]).GetAtomicNum()
        atom.vdw_radius = ob.GetVdwRad(atomic_num)
