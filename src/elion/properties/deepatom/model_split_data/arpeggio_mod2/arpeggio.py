...
''', formatter_class=argparse.RawTextHelpFormatter)
    ...
    def get_ligand_element(atom):
        # print('atom.name: %s' % atom.name)
        try:
            if atom.name[1].isalpha():
                lig_element = atom.name[:2]
            else:
                lig_element = atom.name[0]
        except:
            print("Element not defined neither in ligand elements")
            print("nor in protein ANOLEA atom types.")
            print("Maybe a terminal atom like OXT")
