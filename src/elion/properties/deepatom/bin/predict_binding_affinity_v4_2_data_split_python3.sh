...
#------------------------------------------------------------------------
#       Script written by: Mohammad A. Rezaei [2020.02.10]
...
DATASET_DIR="${DRUG_ROOT_DIR}/${input_type}/${dataset_name}"
...
do
     ...
     then
          ...
          python "${PRE_DIR}/01_preprocess_complexes_VS_python3.py"  "${batch_dir}"

          /bin/bash "${PRE_DIR}/02a_VS__Non_augmented_data_make_directory.sh"  -d "${batch_dir}"

          module load conda chimera
          /bin/bash "${PRE_DIR}/02b_augment_in_Chimera_VS_opt.sh"  -d "${batch_dir}"  -p "${PRE_DIR}"
          module unload chimera
          ...
          python "${GEN_DIR}/make_grid_for_aug_mp_python3.py"  "${batch_dir}"  "${dataset_name}"
